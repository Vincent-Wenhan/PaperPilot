...
def build_backend_adapter_source(
    template_type: str,
    repo_path: str | Path,
    prototype_plan: dict[str, Any] | None = None,
) -> str:
    """Return a Python backend adapter with mock-first predict behavior."""
    template = _normalize_template(template_type)
    prototype_plan = prototype_plan or {}
    mock_result = prototype_plan.get("mock_result")
    if isinstance(mock_result, dict) and mock_result:
    ...
    payload = {
        ...
        "adapter_boundary": prototype_plan.get("adapter_boundary", []),
        "real_integration_placeholder": prototype_plan.get(
            "real_integration_placeholder",
            "Review repository inference code before disabling mock mode.",
        ),
    }
    return f'''"""Mock-first backend adapter generated by PaperPilot."""
...
ADAPTER_CONFIG = {_json_script(payload)}


class ModelAdapter:
    """Backend adapter boundary for the generated product."""

    def __init__(self, repo_path: str = "", mock_mode: bool = True) -> None:
        self.repo_path = repo_path or str(ADAPTER_CONFIG["repo_path"])
    ...
    def setup(self) -> dict[str, Any]:
        return {{
            ...
            "template": ADAPTER_CONFIG["template"],
        }}
    ...
    def load_model(self) -> None:
        ...
        raise NotImplementedError(
            ADAPTER_CONFIG["real_integration_placeholder"]
        )
    ...
    def predict(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        result = copy.deepcopy(ADAPTER_CONFIG["base_result"])
        result["mock_mode"] = True
        result["input_signature"] = self._signature(payload)
        result["input_summary"] = {{
            key: str(value)[:160] for key, value in sorted(payload.items())
        }}
        return result
'''


def build_backend_main_source() -> str:
    """Return a FastAPI app that exposes health and predict endpoints."""
    return '''"""FastAPI backend for the generated PaperPilot product."""
...
from backend.adapter import ModelAdapter


app = FastAPI(title="Generated PaperPilot Product Backend")
...
def health() -> dict[str, Any]:
    return {"status": "ok", "adapter": adapter.setup()}


@app.post("/predict")
def predict(request: PredictRequest) -> dict[str, Any]:
    return adapter.predict(request.inputs)
'''


def build_requirements_source(dependencies: list[dict[str, Any]] | None = None) -> str:
    """Return Python dependencies for the generated backend."""
    ...
    for item in requested:
        if str(item.get("kind") or "python").lower() != "python":
            continue
        name = str(item.get("name") or "").strip()
        if not name:
            continue
        python_deps[name] = str(item.get("version") or "").strip()
    return "\n".join(
        f"{name}{version}" if version else name
        for name, version in sorted(python_deps.items())
    )
...
def build_static_bundle_sources(
    ...
    """Return the complete manifest-driven product bundle."""
    prototype_plan = prototype_plan or {}
    return {
        "frontend/index.html": build_index_source(
            template_type,
            product_spec,
            frontend_plan,
        ...
        "frontend/app.js": build_client_source(
            template_type,
            product_spec,
            frontend_plan,
            prototype_plan=prototype_plan,
        ...
        "frontend/adapter.js": build_adapter_source(
            template_type,
            repo_path,
            prototype_plan=prototype_plan,
        ),
        "frontend/styles.css": build_styles_source(),
        "backend/__init__.py": "",
        "backend/main.py": build_backend_main_source(),
        "backend/adapter.py": build_backend_adapter_source(
            template_type,
            repo_path,
            prototype_plan=prototype_plan,
        ),
        "requirements.txt": build_requirements_source(
            prototype_plan.get("dependencies")
            if isinstance(prototype_plan.get("dependencies"), list)
            else None
        ),
    }
