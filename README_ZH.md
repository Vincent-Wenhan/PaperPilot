...
旧 Productize Agent 继续保留用于兼容，但升级后的流水线使用以上四个高层 Agent。所有 LLM Agent 共用 `BaseAgent` 和统一的 OpenAI-compatible `LLMClient`。命令执行与仓库 clone 不由 LLM 决策。
...
6. 点击 **Generate Product Prototype**。
7. 查看能力卡、组合计划、产品机会、PRD/MVP、原型计划、生成文件和评估。

Productize 流水线会返回 `capability_cards`、`capability_map`、
`composition_plan`、`product_plan`、`prd`、`mvp_scope`、
`prototype_plan` 和 `evaluation` 等结构化产物，同时保持旧的单论文
`run_productize_pipeline()` 调用兼容。
