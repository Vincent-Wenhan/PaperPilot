...
PaperPilot 是一个面向科研新手的多智能体论文复现助手。传统论文复现通常需要研究者反复阅读论文、查找代码、配置环境、理解实验设置并解决报错，过程复杂且容易失败。本项目将论文复现流程拆分为论文阅读、方法拆解、代码获取或生成、环境配置、实验规划、运行诊断和报告生成等多个阶段，并设计多个专业 Agent 协作完成这些任务。用户只需要上传论文 PDF，可选提供 GitHub 仓库链接；没有链接时，Code Agent 会根据论文分析生成最小复现项目。
...
└── Reproduction Goal
↓
PDF Parser +（GitHub Clone 或 Code Agent）+ Repo Scanner
↓
