...
```
User Input（单篇或多篇论文）→ PDF Parser +（可选 GitHub Clone 或 Code Agent）+ Repo Scanner
  → Reproduce Multi-Agent Pipeline
  → Outputs（三份复现文件）
...
- **少量高层 Agent**：Productize 使用 Research Synthesizer、Product Planner、Prototype Builder、Product Evaluator
...
├── main.py             # 主流程编排入口
├── config.py           # 环境变量配置
├── agents/             # Reproduce 与 Productize Agents
...
├── prompts/            # Agent 提示词文件
├── workspace/          # clone 的仓库或 Code Agent 生成的项目
├── uploads/            # 上传的 PDF
├── outputs/            # 生成的输出文件
...
- Code Agent 生成文件只允许写入独立的 `workspace/generated_reproduction_*` 目录
- Code Agent 拒绝绝对路径、父目录穿越、`.git`、保留文件名、重复路径和超出大小限制的输出
...
- Product Tester 只做文件、文本标记和 Python 语法检查，不启动 Streamlit
